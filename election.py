...
class Voter:
    def __init__(self, secmen_id, sifre, secmen_il, secmen_ilce, oy_kullandi_mi=False):
        self.secmen_id = secmen_id
        self.sifre = sifre
        self.secmen_il = secmen_il
        self.secmen_ilce = secmen_ilce
        self.oy_kullandi_mi = oy_kullandi_mi
...
class Blokzincir:
    ...
    def load_secmenler(self):
        connection = sqlite3.connect('database.db')
        cursor = connection.cursor()
        cursor.execute("SELECT secmen_id, sifre, secmen_il, secmen_ilce, oy_kullandi_mi FROM secmenler")
        secmenler = cursor.fetchall()
        connection.close()
        return {secmen_id: Voter(secmen_id, sifre, secmen_il, secmen_ilce, oy_kullandi_mi) for secmen_id, sifre, secmen_il, secmen_ilce, oy_kullandi_mi in secmenler}
    ...
    def oy_ekle(self, oy, secmen_id, sifre):
        ...
        if self.secmenler[secmen_id].oy_kullandi_mi:
            return "Bu seçmen zaten oy kullandı!"
        ...
        self.secmenler[secmen_id].oy_kullandi_mi = True  # Oy kullanma durumu güncelleniyor
        
        # Veritabanında oy_kullandi_mi alanını güncelle
        connection = sqlite3.connect('database.db')
        cursor = connection.cursor()
        cursor.execute("UPDATE secmenler SET oy_kullandi_mi = TRUE WHERE secmen_id = ?", (secmen_id,))
        connection.commit()
        connection.close()
        
        if len(self.pending_votes) >= 2:  # Örnek olarak 2 oy biriktiğinde yeni bir blok oluşturulacak
            self.blok_guncelle()
...
def vote():
    ...
    secmen = blok_zincirim.secmenler.get(secmen_id)

    if secmen is None:
        flash("Geçersiz seçmen ID'si!", "danger")
        return redirect(url_for('user_login'))

    if secmen.oy_kullandi_mi:
...
def aday_guncelle_post(aday_id):
    ...
    conn.commit()
    return redirect(url_for('aday_islemleri'))
@app.route('/secmen_islemleri')
def secmen_islemleri():
    if 'admin' in session:
        connection = get_db_connection()
        secmenler = connection.execute('SELECT * FROM secmenler').fetchall()
        connection.close()
        return render_template('secmen_islemleri.html', secmenler=secmenler)
    else:
        flash("Yetkisiz erişim!", "danger")
        return redirect(url_for('admin_login'))

@app.route('/secmen_ekle', methods=['POST'])
def secmen_ekle():
    secmen_id = request.form['secmen_id']
    sifre = request.form['sifre']
    secmen_il = request.form['secmen_il']
    secmen_ilce = request.form['secmen_ilce']
    connection = get_db_connection()
    connection.execute('INSERT INTO secmenler (secmen_id, sifre, secmen_il, secmen_ilce) VALUES (?, ?, ?, ?)', (secmen_id, sifre, secmen_il, secmen_ilce))
    connection.commit()
    connection.close()
    return redirect(url_for('secmen_islemleri'))

@app.route('/secmen_sil/<int:secmen_id>')
def secmen_sil(secmen_id):
    connection = get_db_connection()
    connection.execute('DELETE FROM secmenler WHERE secmen_id = ?', (secmen_id,))
    connection.commit()
    connection.close()
    return redirect(url_for('secmen_islemleri'))

@app.route('/secmen_guncelle/<int:secmen_id>')
def secmen_guncelle(secmen_id):
    connection = get_db_connection()
    secmen = connection.execute('SELECT * FROM secmenler WHERE secmen_id = ?', (secmen_id,)).fetchone()
    connection.close()
    return render_template('secmen_guncelle.html', secmen=secmen)

@app.route('/secmen_guncelle/<int:secmen_id>', methods=['POST'])
def secmen_guncelle_post(secmen_id):
    sifre = request.form['sifre']
    secmen_il = request.form['secmen_il']
    secmen_ilce = request.form['secmen_ilce']
    connection = get_db_connection()
    connection.execute('UPDATE secmenler SET sifre = ?, secmen_il = ?, secmen_ilce = ? WHERE secmen_id = ?', (sifre, secmen_il, secmen_ilce, secmen_id))
    connection.commit()
    connection.close()
    return redirect(url_for('secmen_islemleri'))
